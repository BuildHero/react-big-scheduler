...
var SchedulerData = function () {
    function SchedulerData() {
        var date = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : (0, _moment2.default)().format(_index3.DATE_FORMAT);
        var viewType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : _index3.ViewTypes.Week;
        ...
        this.cellUnit = viewType === _index3.ViewTypes.Day ? _index3.CellUnits.Hour : _index3.CellUnits.Day;
        this.showAgenda = showAgenda;
        this.isEventPerspective = isEventPerspective;
        this.resizing = false;
        this.scrollToSpecialMoment = false;
        this.documentWidth = 0;
        this.initRenderData = null;
    ...
    _createClass(SchedulerData, [{
        ...
        key: 'setResourceNameTemplate',
        value: function setResourceNameTemplate(resourceTemplate) {
            this.config.resourceName = resourceTemplate;
            this._createRenderData();
        }
    }, {
        key: 'setHeaderTemplate',
        value: function setHeaderTemplate(headerTemplate) {
            this.config.headerTemplate = headerTemplate;
            this._createRenderData();
        }
    }, {
        ...
        value: function setViewType() {
            var viewType = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : _index3.ViewTypes.Week;
            ...
            this.showAgenda = showAgenda;
            this.isEventPerspective = isEventPerspective;
            this.cellUnit = _index3.CellUnits.Day;

            if (this.viewType !== viewType) {
                var date = this.startDate;

                if (viewType === _index3.ViewTypes.Custom || viewType === _index3.ViewTypes.Custom1 || viewType === _index3.ViewTypes.Custom2) {
                    this.viewType = viewType;
                    this._resolveDate(0, date);
                } else {
                    if (this.viewType < viewType) {
                        if (viewType === _index3.ViewTypes.Week) {
                            this.startDate = this.localeMoment(date).startOf('week').format(_index3.DATE_FORMAT);
                            this.endDate = this.localeMoment(this.startDate).endOf('week').format(_index3.DATE_FORMAT);
                        } else if (viewType === _index3.ViewTypes.Month) {
                            this.startDate = this.localeMoment(date).startOf('month').format(_index3.DATE_FORMAT);
                            this.endDate = this.localeMoment(this.startDate).endOf('month').format(_index3.DATE_FORMAT);
                        } else if (viewType === _index3.ViewTypes.Quarter) {
                            this.startDate = this.localeMoment(date).startOf('quarter').format(_index3.DATE_FORMAT);
                            this.endDate = this.localeMoment(this.startDate).endOf('quarter').format(_index3.DATE_FORMAT);
                        } else if (viewType === _index3.ViewTypes.Year) {
                            this.startDate = this.localeMoment(date).startOf('year').format(_index3.DATE_FORMAT);
                            this.endDate = this.localeMoment(this.startDate).endOf('year').format(_index3.DATE_FORMAT);
                        }
                    } else {
                        ...
                        if (now >= start && now < end) {
                            date = now.format(_index3.DATE_FORMAT);
                        }

                        if (viewType === _index3.ViewTypes.Day) {
                            this.startDate = date;
                            this.endDate = this.startDate;
                            this.cellUnit = _index3.CellUnits.Hour;
                        } else if (viewType === _index3.ViewTypes.Week) {
                            this.startDate = this.localeMoment(date).startOf('week').format(_index3.DATE_FORMAT);
                            this.endDate = this.localeMoment(this.startDate).endOf('week').format(_index3.DATE_FORMAT);
                        } else if (viewType === _index3.ViewTypes.Month) {
                            this.startDate = this.localeMoment(date).startOf('month').format(_index3.DATE_FORMAT);
        ...
        value: function getCellMaxEvents() {
            return this.viewType === _index3.ViewTypes.Week ? this.config.weekMaxEvents : this.viewType === _index3.ViewTypes.Day ? this.config.dayMaxEvents : this.viewType === _index3.ViewTypes.Month ? this.config.monthMaxEvents : this.viewType === _index3.ViewTypes.Year ? this.config.yearMaxEvents : this.viewType === _index3.ViewTypes.Quarter ? this.config.quarterMaxEvents : this.config.customMaxEvents;
        }
    }, {
        key: 'getDateLabel',
        ...
        value: function addEvent(newEvent) {
            ...
            if (this.eventGroupsAutoGenerated) this._generateEventGroups();
            this._createRenderData(newEvent);
        }
    }, {
        ...
        value: function moveEvent(event, newSlotId, newSlotName, newStart, newEnd) {
            ...
            event.start = newStart;
            this._attachEvent(event);
            this._createRenderData(event, oldSlot);
        }
    }, {
        ...
        value: function getResourceTableConfigWidth() {
            ...
            return this.viewType === _index3.ViewTypes.Week ? this.config.weekResourceTableWidth : this.viewType === _index3.ViewTypes.Day ? this.config.dayResourceTableWidth : this.viewType === _index3.ViewTypes.Month ? this.config.monthResourceTableWidth : this.viewType === _index3.ViewTypes.Year ? this.config.yearResourceTableWidth : this.viewType === _index3.ViewTypes.Quarter ? this.config.quarterResourceTableWidth : this.config.customResourceTableWidth;
        }
    }, {
        key: 'getContentCellConfigWidth',
        value: function getContentCellConfigWidth() {
            return this.viewType === _index3.ViewTypes.Week ? this.config.weekCellWidth : this.viewType === _index3.ViewTypes.Day ? this.config.dayCellWidth : this.viewType === _index3.ViewTypes.Month ? this.config.monthCellWidth : this.viewType === _index3.ViewTypes.Year ? this.config.yearCellWidth : this.viewType === _index3.ViewTypes.Quarter ? this.config.quarterCellWidth : this.config.customCellWidth;
        }
    }, {
            ...
            recurringEvents.forEach(function (item) {
                ...
                var newEvents = all.map(function (time, index) {
                    return _extends({}, item, {
                        ...
                        id: item.id + '-' + index,
                        start: _this2.localeMoment(time).format(_index3.DATETIME_FORMAT),
                        end: _this2.localeMoment(time).add(oldEnd.diff(oldStart), 'ms').format(_index3.DATETIME_FORMAT)
                    });
                });
        ...
        value: function _resolveDate(num) {
            ...
            if (date != undefined) this.selectDate = this.localeMoment(date).format(_index3.DATE_FORMAT);

            if (this.viewType === _index3.ViewTypes.Week) {
                this.startDate = date != undefined ? this.localeMoment(date).startOf('week').format(_index3.DATE_FORMAT) : this.localeMoment(this.startDate).add(num, 'weeks').format(_index3.DATE_FORMAT);
                this.endDate = this.localeMoment(this.startDate).endOf('week').format(_index3.DATE_FORMAT);
            } else if (this.viewType === _index3.ViewTypes.Day) {
                this.startDate = date != undefined ? this.selectDate : this.localeMoment(this.startDate).add(num, 'days').format(_index3.DATE_FORMAT);
                this.endDate = this.startDate;
            } else if (this.viewType === _index3.ViewTypes.Month) {
                this.startDate = date != undefined ? this.localeMoment(date).startOf('month').format(_index3.DATE_FORMAT) : this.localeMoment(this.startDate).add(num, 'months').format(_index3.DATE_FORMAT);
                this.endDate = this.localeMoment(this.startDate).endOf('month').format(_index3.DATE_FORMAT);
            } else if (this.viewType === _index3.ViewTypes.Quarter) {
                this.startDate = date != undefined ? this.localeMoment(date).startOf('quarter').format(_index3.DATE_FORMAT) : this.localeMoment(this.startDate).add(num, 'quarters').format(_index3.DATE_FORMAT);
                this.endDate = this.localeMoment(this.startDate).endOf('quarter').format(_index3.DATE_FORMAT);
            } else if (this.viewType === _index3.ViewTypes.Year) {
                this.startDate = date != undefined ? this.localeMoment(date).startOf('year').format(_index3.DATE_FORMAT) : this.localeMoment(this.startDate).add(num, 'years').format(_index3.DATE_FORMAT);
                this.endDate = this.localeMoment(this.startDate).endOf('year').format(_index3.DATE_FORMAT);
            } else if (this.viewType === _index3.ViewTypes.Custom || this.viewType === _index3.ViewTypes.Custom1 || this.viewType === _index3.ViewTypes.Custom2) {
        ...
        value: function _createHeaders() {
            ...
            if (this.showAgenda) {
                headers.push({ time: header.format(_index3.DATETIME_FORMAT), nonWorkingTime: false });
            } else {
                if (this.cellUnit === _index3.CellUnits.Hour) {
                    ...
                    while (header >= start && header <= end) {
                        ...
                        for (var i = 0; i < minuteSteps; i++) {
                            ...
                            if (hour >= this.config.dayStartFrom && hour <= this.config.dayStopTo) {
                                var time = header.format(_index3.DATETIME_FORMAT);
                    ...
                    while (header >= start && header <= end) {
                        var _time = header.format(_index3.DATETIME_FORMAT);
        ...
        value: function _createInitHeaderEvents(header) {
            var start = this.localeMoment(header.time),
                startValue = start.format(_index3.DATETIME_FORMAT);
            var endValue = this.showAgenda ? this.viewType === _index3.ViewTypes.Week ? start.add(1, 'weeks').format(_index3.DATETIME_FORMAT) : this.viewType === _index3.ViewTypes.Day ? start.add(1, 'days').format(_index3.DATETIME_FORMAT) : this.viewType === _index3.ViewTypes.Month ? start.add(1, 'months').format(_index3.DATETIME_FORMAT) : this.viewType === _index3.ViewTypes.Year ? start.add(1, 'years').format(_index3.DATETIME_FORMAT) : this.viewType === _index3.ViewTypes.Quarter ? start.add(1, 'quarters').format(_index3.DATETIME_FORMAT) : this.localeMoment(this.endDate).add(1, 'days').format(_index3.DATETIME_FORMAT) : this.cellUnit === _index3.CellUnits.Hour ? start.add(this.config.minuteStep, 'minutes').format(_index3.DATETIME_FORMAT) : start.add(1, 'days').format(_index3.DATETIME_FORMAT);
            return {
                time: header.time,
            ...
            try {
                for (var _iterator = headers[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    ...
                    var spanStart = this.localeMoment(header.time),
                        spanEnd = this.cellUnit === _index3.CellUnits.Hour ? this.localeMoment(header.time).add(this.config.minuteStep, 'minutes') : this.localeMoment(header.time).add(1, 'days');
        ...
        value: function _createRenderData() {
            ...
            var oldSlot = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;

            var initRenderData = this.initRenderData;
            if (item !== null && initRenderData) {
                ...
                if (oldSlot) {
                    ...
                    var oldResourceEventList = initRenderData.filter(function (x) {
                        return x.slotId === oldSlot;
                    });
                    if (oldResourceEventList.length > 0) {
                        var oldResourceEvents = oldResourceEventList[0];
                        var span = this._getSpan(item.start, item.end, this.headers);
                        oldResourceEvents.headerItems.forEach(function (header, index) {
                            for (var i = 0; i < header.events.length; i++) {
                                if (header.events[i] && header.events[i].eventItem.id === item.id) {
                                    header.events[i] = _this5._createHeaderEvent(false, span, item);
                                }
                            }
                        });
                    }
                ...
                if (resourceEventsList.length > 0) {
                    ...
                    var _span = this._getSpan(item.start, item.end, this.headers);
                    var eventStart = this.localeMoment(item.start),
                        eventEnd = this.localeMoment(item.end);
                    var pos = -1;
                    for (var k = 0; k < resourceEvents.headerItems.length; k++) {
                        var header = resourceEvents.headerItems[k];
                        var index = k;
                        for (var i = 0; i < header.events.length; i++) {
                            if (header.events[i] && header.events[i].eventItem.id === item.id) {
                                header.events[i] = this._createHeaderEvent(false, _span, item);
                            }
                        }
                        var headerStart = this.localeMoment(header.start),
                        ...
                        if (headerEnd > eventStart && headerStart < eventEnd) {
                            ...
                            if (render === false) {
                                ...
                                var previousHeaderStart = this.localeMoment(previousHeader.start),
                            ...
                            header.events[pos] = this._createHeaderEvent(render, _span, item);
                        }
                    }
                }
            } else {
                ...
                this.events = (0, _lodash.uniqBy)(this.events, function (e) {
                    return e.id;
                });
                // this.events.forEach((item) => {
                if (this.events && this.events.length) {
                    var _loop = function _loop(_i) {
                        var item = _this5.events[_i];
                        var resourceEventsList = initRenderData.filter(function (x) {
                            return x.slotId === _this5._getEventSlotId(item);
                        });
                        if (resourceEventsList.length > 0) {
                            var _resourceEvents = resourceEventsList[0];
                            var _span2 = _this5._getSpan(item.start, item.end, _this5.headers);
                            var _eventStart = _this5.localeMoment(item.start),
                                _eventEnd = _this5.localeMoment(item.end);
                            var _pos = -1;
                            // resourceEvents.headerItems.forEach((header, index) => {
                            for (var _k = 0; _k < _resourceEvents.headerItems.length; _k++) {
                                var _header = _resourceEvents.headerItems[_k];
                                var _index = _k;
                                var _headerStart = _this5.localeMoment(_header.start),
                                    _headerEnd = _this5.localeMoment(_header.end);
                                if (_headerEnd > _eventStart && _headerStart < _eventEnd) {
                                    _header.count = _header.count + 1;

                                    var _tmp = _pos;
                                    if (_tmp === -1) _tmp = 0;
                                    while (_header.events[_tmp] !== undefined) {
                                        _tmp++;
                                    }
                                    _pos = _tmp;

                                    var _render = _headerStart <= _eventStart || _index === 0;
                                    if (_render === false) {
                                        var _previousHeader = _resourceEvents.headerItems[_index - 1];
                                        var _previousHeaderStart = _this5.localeMoment(_previousHeader.start),
                                            _previousHeaderEnd = _this5.localeMoment(_previousHeader.end);
                                        if (_previousHeaderEnd <= _eventStart || _previousHeaderStart >= _eventEnd) _render = true;
                                    }
                                    _header.events[_pos] = _this5._createHeaderEvent(_render, _span2, item);
                                }
                            }
                        }
                    };
            ...
            for (var j = 0; j < initRenderData.length; j++) {
                var _resourceEvents2 = initRenderData[j];
                var maxRowsCount = 0;
                var hasSummary = false;
                // resourceEvents.headerItems.forEach((headerItem) => {
                for (var _k2 = 0; _k2 < _resourceEvents2.headerItems.length; _k2++) {
                    var headerItem = _resourceEvents2.headerItems[_k2];
                    ...
                    if (this.behaviors.getSummaryFunc !== undefined) {
                        (function () {
                            ...
                            headerItem.summary = _this5.behaviors.getSummaryFunc(_this5, events, _resourceEvents2.slotId, _resourceEvents2.slotName, headerItem.start, headerItem.end);
                            if (!!headerItem.summary && headerItem.summary.text != undefined) hasSummary = true;
                        })();
                    }
                };

                _resourceEvents2.hasSummary = hasSummary;
                var rowsCount = maxRowsCount > this.getCellMaxEvents() ? this.getCellMaxEvents() : maxRowsCount;
                _resourceEvents2.rowHeight = rowsCount === 0 ? this.config.eventItemLineHeight + 2 : rowsCount * this.config.eventItemLineHeight + (this.config.creatable && this.config.checkConflict === false ? 20 : 2);
                if (hasSummary) _resourceEvents2.rowHeight = _resourceEvents2.rowHeight + this.config.eventItemLineHeight;
                if (this.config.nonAgendaSlotMinHeight !== 0 && _resourceEvents2.rowHeight < this.config.nonAgendaSlotMinHeight) _resourceEvents2.rowHeight = this.config.nonAgendaSlotMinHeight;
            };

            this.renderData = initRenderData.filter(function (o) {
                return o.render;
            });
        }
    }, {
