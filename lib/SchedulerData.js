    ...
    _createClass(SchedulerData, [{
        ...
        value: function _getSpan(startTime, endTime, headers) {
            ...
            var start = this.localeMoment(startTime),
                ...
                span = 0;
            var hoursTillStart = start.hour();
            if (hoursTillStart > 0) hoursTillStart -= 1;
            var spansToIgnore = hoursTillStart * parseInt(60 / this.config.minuteStep);
            // for(let header of headers) {
            for (var i = spansToIgnore; i < headers.length; i++) {
        ...
        value: function _createRenderData() {
            ...
            if (this.events && this.events.length) {
                var _loop = function _loop(i) {
                    ...
                    if (resourceEventsList.length > 0) {
                        ...
                        var hoursTillStart = eventStart.hour();
                        if (hoursTillStart > 0) hoursTillStart -= 1;
                        var spansToIgnore = hoursTillStart * parseInt(60 / _this5.config.minuteStep);
                        for (var k = spansToIgnore; k < resourceEvents.headerItems.length; k++) {
            ...
            for (var j = 0; j < initRenderData.length; j++) {
                ...
                if (hasSummary) resourceEvents.rowHeight = resourceEvents.rowHeight + this.config.eventItemLineHeight;
                if (this.config.nonAgendaSlotMinHeight !== 0 && resourceEvents.rowHeight < this.config.nonAgendaSlotMinHeight) resourceEvents.rowHeight = this.config.nonAgendaSlotMinHeight;
            };
