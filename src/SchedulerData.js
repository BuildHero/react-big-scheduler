...
export default class SchedulerData {
    ...
    setResourceNameTemplate(resourceTemplate) {
    ...
    _createRenderData(item = null, oldSlot=null) {
        ...
        for(let j=0; j < initRenderData.length; j++){
            let resourceEvents = initRenderData[j];
            let maxRowsCount = 0;
            let hasSummary = false;
            // resourceEvents.headerItems.forEach((headerItem) => {
            for(let k=0; k< resourceEvents.headerItems.length; k++){
                let headerItem = resourceEvents.headerItems[k];
        ...
        this.renderData = initRenderData.filter(o => o.render);
    }
